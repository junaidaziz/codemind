...
Format your response as JSON: {
  ...
  "testingPlan": "how to test the fix"
    ...
    await prisma.autoFixSession.update({
      ...
      data: {
        ...
        proposedSolution: JSON.stringify(analysis),
   ...
   */
  private async generateCodeFixes(sessionId: string, analysis: any, config: APRConfig) {
...
Format as JSON: {
  ...
  "dependencies": []
    ...
    const attempt = await prisma.autoFixAttempt.create({
      data: {
        sessionId,
        attemptNumber: 1,
        filesModified: codeChanges.changes?.map((c: any) => c.file) || [],
        prompt,
   ...
   */
  private async validationLoop(sessionId: string, codeGeneration: any, config: APRConfig) {
    ...
    while (retryCount <= (config.maxRetries || 3) && !validationPassed) {
      ...
      if (validationResult.passed) {
        ...
        await prisma.autoFixSession.update({
          ...
          data: {
            validationPassed: true,
            lintPassed: validationResult.lint.passed,
            typeCheckPassed: validationResult.typecheck.passed,
            unitTestsPassed: validationResult.unitTests.passed,
            e2eTestsPassed: validationResult.e2eTests.passed
          }
        });
        break;
      }
    ...
    for (const step of validationSummary.steps) {
      await prisma.autoFixValidation.create({
        data: {
          sessionId,
          attemptNumber,
          validationType: step.step === 'tests' ? 'UNIT_TEST' : step.step.toUpperCase() as any,
          passed: step.success,
          output: step.output,
...
Format as JSON: {
  ...
  "fixes": [
    {
      ...
      await prisma.autoFixAttempt.create({
        data: {
          sessionId,
          attemptNumber,
          filesModified: healPatch.fixes?.map((f: any) => f.file) || [],
          prompt,
          aiResponse: response.choices[0].message.content || '',
   ...
   */
  private async performAIReview(sessionId: string, codeGeneration: any): Promise<number> {
    const prompt = `Review this automatically generated code fix and identify potential issues:

Modified Files: ${codeGeneration.filesModified.join(', ')}

Code Changes:
${JSON.stringify(codeGeneration.changes, null, 2)}
  ...
  "findings": [
    ...
    for (const finding of findings) {
      await prisma.autoFixReview.create({
        data: {
          sessionId,
          reviewType: finding.type as any,
...
### Validation Results
- ✅ Lint: ${session.lintPassed ? 'Passed' : 'N/A'}
- ✅ TypeCheck: ${session.typeCheckPassed ? 'Passed' : 'N/A'}
- ✅ Unit Tests: ${session.unitTestsPassed ? 'Passed' : 'N/A'}
- ${session.retryCount > 0 ? `🔄 Self-healed in ${session.retryCount} ${session.retryCount === 1 ? 'attempt' : 'attempts'}` : '✅ No retries needed'}
...
*Validation: ${session.validationPassed ? 'Passed' : 'Manual review required'}*`;
    ...
    const { data: pr } = await octokit.pulls.create({
      ...
      base: defaultBranch,
      draft: reviewFindings > 0 || !session.validationPassed // Draft if issues found
    });
  ...
  private async postReviewComments(sessionId: string, prNumber: number, config: APRConfig) {
    ...
    const summaryByCategory = reviews.reduce((acc: any, r) => {
...
${reviews.map(r => `${severityEmoji[r.severity]} **${r.severity}**: ${r.issue} (${r.filePath}:${r.lineNumber || '?'})`).join('\n')}
...
*Review each finding below for detailed explanations and suggestions.*`;
    ...
    for (const review of reviews) {
      const commentBody = `## ${severityEmoji[review.severity]} ${review.reviewType.replace(/_/g, ' ')}
...
*Category: ${review.category} | Severity: ${review.severity}*`;
   ...
   * Helper methods
   */
  private async updatePhase(sessionId: string, status: string, phase: string) {
    await prisma.autoFixSession.update({
      where: { id: sessionId },
      data: { status: status as any, phase: phase as any, updatedAt: new Date() }
    });
  }
